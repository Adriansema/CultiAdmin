...
class PqrsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $pqrs = Pqrs::create([
            'user_id' => Auth::id(), // Si el usuario esta autenticado, su ID; de lo contrario, null
        ...
        // **USO DE $pqrs: ENVIAR EL CORREO DE CONFIRMACIoN**
        try {
            ...
            // Es buena practica capturar excepciones para no romper la aplicacion si el envio falla
            // Puedes loggear el error o notificar a un administrador
            \Log::error("Error al enviar correo de confirmacion de PQR (ID: {$pqrs->id}): " . $e->getMessage());
        }

        return redirect()->back()->with('success', 'Tu PQR ha sido enviado exitosamente. Revisamos tu solicitud y te enviaremos una confirmacion a tu correo.');
    }
}