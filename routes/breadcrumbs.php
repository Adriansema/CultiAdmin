...
/* use Diglactic\Breadcrumbs\Generator as ; */
...
Breadcrumbs::for('operador.pendientes', function ( $trail) {
    $trail->parent('dashboard')
        ->push('Operador', route('operador.pendientes'))
        ->push('Pendientes');
});
...
Breadcrumbs::for('operador.productos.show', function ( $trail, $producto) {
    $trail->parent('operador.pendientes')
        ->push('Operador', route('operador.pendientes'))
        ->push('Detalle de Producto', route('operador.productos.show', $producto));
});

Breadcrumbs::for('operador.boletines.show', function ( $trail, $boletin) {
    $trail->parent('operador.pendientes')
        ->push('Operador', route('operador.pendientes'))
        ->push('Detalle de Boletín', route('operador.boletines.show', $boletin));
});

Breadcrumbs::for('operador.historial.index', function ( $trail) {
    $trail->parent('dashboard')
        ->push('Operador', route('operador.pendientes'))
        ->push('Historial', route('operador.historial.index'));
}); 