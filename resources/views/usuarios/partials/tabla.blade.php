                 ...
                 @foreach ($usuarios as $usuario)
                     <tr class="bg-white hover:bg-gray-300">
                         <td class="px-6 py-4 flex items-center group relative">
                             ...
                             {{-- Botón de edición: Visible solo si el usuario puede 'actualizar' este usuario específico --}}
                             @can('update', $usuario)
                         ...
                         <td class=" py-4">
                             {{-- Dentro del bucle foreach para cada $usuario 
                             <p>Usuario Actual ID: {{ $usuario->id }}</p>
                             <p>Usuario Logueado ID: {{ Auth::id() }}</p>
                             <p>¿IDs Iguales?: {{ Auth::id() === $usuario->id ? 'Sí' : 'No' }}</p>
                             @php
                                 // Llama directamente a la Policy para ver el resultado
                                 $policyAllowsToggle = Auth::user()->can('toggle', $usuario);
                             @endphp
                             <p>Policy permite toggle: {{ $policyAllowsToggle ? 'Sí' : 'No' }}</p>--}}
                             {{-- Formulario para cambiar estado: Visible solo si el usuario puede 'toggleStatus' este usuario específico --}}
                             @can('toggle', $usuario)
