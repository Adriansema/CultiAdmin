...
@section('content')
    ...
    @can('crear usuario')
        ...
        <div class="w-full max-w-screen-2xl mx-auto bg-[var(--color-Gestion)] rounded-3xl p-4 mb-8 shadow-sm"> {{-- Añadido shadow-sm para consistencia --}}
            ...
            <div class="flex flex-col justify-between gap-4 mb-4 md:flex-row md:items-center"> {{-- Se convierte en columna en móvil, y fila en md+ --}}
                ...
                <div class="flex flex-col justify-end w-full py-2 space-y-2 sm:flex-row sm:items-center sm:py-5 sm:space-y-0 sm:space-x-2"> {{-- space-y-2 para apilar, space-x-2 para fila --}}
                    ...
                    <form method="GET" action="{{ route('usuarios.exportar') }}" id="exportCsvForm" class="w-full sm:w-auto"> {{-- w-full en móvil, w-auto en sm+ --}}
                        <button type="submit" id="exportCsvButton"
                            ...
                            <img src="{{ asset('images/export.svg') }}"
                                class="relative inset-0 block w-5 h-4 group-hover:hidden" alt="Icono Exportar CSV"> {{-- Ajuste de tamaño de imagen --}}
                            <img src="{{ asset('images/export-hover.svg') }}"
                                class="relative inset-0 hidden w-5 h-4 group-hover:block" alt="Icono Exportar CSV"> {{-- Ajuste de tamaño de imagen --}}
                        </button>
                    </form>
                    ...
                    @if (Auth::user()->hasAnyRole(['SuperAdmin', 'Administrador']))
                        ...
                        hover:bg-[#61BA33] text-white rounded-full w-full sm:w-auto create-user-button"> {{-- w-full en móvil, w-auto en sm+ --}}
                            <img src="{{ asset('images/signo.svg') }}" class="w-4 h-3 mr-3" alt="Icono Nuevo Usuario"> {{-- Añadido mr-3 para espacio --}}
            ...
            {{-- Aquí se incluye la tabla. Asegúrate de que el partial 'usuarios.partials.tabla' tenga el 'overflow-x-auto' y 'min-w-max' --}}
