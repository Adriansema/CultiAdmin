                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 border">
                                    @if ($producto->imagen)
                                        <img src="{{ asset('storage/' . $producto->imagen) }}"
                                            class="object-cover w-24 h-24 rounded" alt="Imagen de {{ $producto->nombre }}">
                                    @else
                                ...
                                <td class="px-4 py-2 space-y-1 border">
                                    <a href="{{ route('operador.productos.show', $producto->id) }}"
                                        class="w-full inline-block px-4 py-2 mb-1 text-white bg-blue-600 rounded hover:bg-blue-700 text-center">Ver</a>

                                    <form action="{{ route('operador.productos.validar', $producto->id) }}" method="POST"
                                        onsubmit="return confirm('¿Estás seguro de validar este producto?')">
                                        @csrf
                                        <x-button class="w-full bg-green-600 hover:bg-green-700">Validar</x-button>
                                    </form>

                                    <button onclick="mostrarModal('producto', '{{ $producto->id }}')"
                                        class="w-full px-4 py-2 mt-1 text-white bg-red-600 rounded hover:bg-red-700">Rechazar</button>
                                    <!-- Modal de rechazo -->
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 space-y-1 border">
                                    ...
                                    <form action="{{ route('operador.boletines.validar', $boletin->id) }}" method="POST"
                                        onsubmit="return confirm('¿Estás seguro de validar este boletín?')">
                                        @csrf
                                        <x-button class="w-full bg-green-600 hover:bg-green-700">Validar</x-button>
                                    </form>

                                    <button onclick="mostrarModal('boletin', '{{ $boletin->id }}')"
                                        class="w-full px-4 py-2 mt-1 text-white bg-red-600 rounded hover:bg-red-700">Rechazar</button>
                                    <!-- Modal de rechazo -->
