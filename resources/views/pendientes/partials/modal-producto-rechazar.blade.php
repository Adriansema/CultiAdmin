<!-- Modal de rechazo -->
<div id="modal-rechazar-productos-{{ $producto->id }}"
    ...
    <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
        ...
        <form action="{{ route('pendientes.productos.rechazar', $producto->id) }}" method="POST">
            @csrf
            <textarea name="observaciones" class="w-full p-2 border border-gray-300 rounded-md mb-8" rows="4"></textarea>
            @error('observaciones')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
            <div class="flex items-center justify-between">
                <button type="button" onclick="cerrarModal('rechazar-productos', '{{ $producto->id }}')"
