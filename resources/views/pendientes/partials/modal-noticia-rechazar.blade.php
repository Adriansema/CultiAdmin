<!-- Modal de rechazo -->
<div id="modal-rechazar-noticia-{{ $noticia->id_noticias }}" class="hidden">
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
            <h3 class="mb-4 text-lg font-bold text-gray-800">Observaciones del rechazo
            </h3>
            <form action="{{ route('pendientes.noticias.rechazar', $noticia->id_noticias) }}" method="POST">
                @csrf
                <textarea name="observaciones" class="w-full p-2 border border-gray-300 rounded-md mb-8" rows="4" required></textarea>
                @error('observaciones')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <div class="flex items-center justify-between">
                    <button type="button" onclick="cerrarModal('rechazar-noticia', '{{ $noticia->id_noticias }}')"
